...
* link:https://mule4-docs.mulesoft.com/connectors/salesforce-connector[Salesforce Connector 9.0.0]
...
* Support for publishing platform event messages: new operation named: Publish Platform Event Message is available
* Added new metadata types
* Provided 3 new operations: Convert Lead, Find Duplicates By Ids, Change Own Password

=== Version 8.4.0 Fixed in this Release
* Apex SOAP call does not work in case that reconnection is triggered*. In previous versions when the session was invalidated the connector throwed an error with this message _Failed to invoke invokeApexSoapMethod_ caused by this exception: _org.apache.cxf.binding.soap.SoapFaul: No operation specified in request (the Body element has no child elements)_. Currently the payload is retained and the calls work fine after the reconnection.
* *There was a leak when the Salesforce connector performed a reconnection*. In previous versions when salesforce connector performed a re-connection HttpClient objects were leaking. In current version there are no leaks when reconnecting.
* *No error received by clients when the _Organization total events daily limit was exceeded_ *. In the previous version when there was an error of type:  _403::Organization total events daily limit exceeded _ the connector throwed no error or Exception. In current version besides throwing an exception a notification will be fired. The _notification id_ for this error will always be _100888_ and it will also contain the appropriate message. 
