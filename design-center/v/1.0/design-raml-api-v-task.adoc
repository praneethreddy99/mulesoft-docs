...
+
Base URI Parameter Name and Type fields appear. Ignore the warning about the unused base URI parameter.
+
...
** Change the default Required setting to unchecked. You do not want to require the user of the API to enter a parameter value. You will define a default in the event a parameter isn't entered.
...
+
. Click GET and then click TRY IT.
+
image::send-test.png[height=321,width=357]
+
. Click Send.
+
A status 200 and comments about the default post 1 are returned.
+
. In URI Parameters, enter a URI parameter of *3* and click Send.
+
A status 200 and comments about post 3 are returned.
+
----
  {
"postId": 3,
